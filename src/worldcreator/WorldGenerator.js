...
define([
    ...
    var WorldGenerator = {
        ...
        prepareWorld: function (seed, worldVO) {
            ...
            worldVO.campPositions = this.generateCampPositions(seed);
            worldVO.passagePositions = this.generatePassagePositions(seed, worldVO.campPositions);
        },
        ...
        generateCampPositions: function (seed) {
            var result = [];
			var topLevel = WorldCreatorHelper.getHighestLevel(seed);
			var bottomLevel = WorldCreatorHelper.getBottomLevel(seed);
            var maxCampDist = 4;
			for (var l = topLevel; l >= bottomLevel; l--) {
                ...
                if (isCampableLevel) {
                    ...
                    if (l != 13) {
                        firstPos = WorldCreatorRandom.randomSectorPosition(seed % 10 + (l+10) * 55, l, maxCenterDist);
                    }
                    positions.push(firstPos);
                    if (l != 13) {
                        var secondPos = WorldCreatorRandom.randomSectorPosition(seed % 100 + (l+5)*10, l, maxCampDist, firstPos, 1);
        ...
        generatePassagePositions: function (seed, campPositions) {
            // TODO ensure not in holes
            // TODO ensure max and min distance from camp pos
            var result = [];
			var topLevel = WorldCreatorHelper.getHighestLevel(seed);
			var bottomLevel = WorldCreatorHelper.getBottomLevel(seed);
			for (var l = topLevel; l >= bottomLevel; l--) {
                ...
                var down = l == bottomLevel ? null : this.getPassagePosition(seed, l, campThisUp, campPosDown);
                result[l] = { up: up, down: down };
            }
            return result;
        },
        
        getPassagePosition: function (seed, level, campPos1, campPos2) {
            ...
            var result = WorldCreatorRandom.randomSectorPosition(seed % 1000 + 7 + (level+13)*101, level, maxDiff, averagePos, minDiff);
