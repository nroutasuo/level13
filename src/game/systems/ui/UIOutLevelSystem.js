define([
    ...
    VisionComponent, PassagesComponent, SectorControlComponent, SectorFeaturesComponent,
	...
	visitedSectors: 0,
        ...
        constructor: function (uiFunctions, gameState, movementHelper, resourceHelper) {
	...
	initVis: function () {
	    ...
	    var sys = this;
	    this.playerLocationNodes.nodeAdded.add( function(node) {
		sys.visitedSectors = rebuildVis(playerPosNodes, sectorNodes);
	    });
            this.visitedSectorNodes.nodeAdded.add( function(node) {
            sys.visitedSectors = rebuildVis(playerPosNodes, sectorNodes);
	    });
	    sys.visitedSectors = rebuildVis(playerPosNodes, sectorNodes);
	},
	...
	updateLevelPage: function() {
	    ...
	    // Improvements		
	    playerActions = this.uiFunctions.playerActions;
	...
	getEnemyDescription: function(isScouted, passagesComponent, hasCampHere) {
	    ...
	    if (hasEnemies) {
			...
			if (window.app) {
				enemyDesc += "(" + enemiesComponent.possibleEnemies + ") ";
			}
	    ...
	    enemyDesc += "Required strength: " + EnemyConstants.getRequiredStength(levelOrdinal) + ". ";
